...
int main(void)
{
	...
	while (offset < 0x5764e)
	{
		...
		} else
		{
			...
			if (oldtable[label])
				...
				sprintf(used[dst]->name, "x%05x_371", dst);
			fprintf(out, "\t\t.dc.w %s-jmpbase\n", used[dst]->name);
		}
		offset += 2;
	}
	...
	fprintf(out, "\n");
	dump_table(fp, 0x5764e, 0x57751);
	fprintf(out, "\n");
	dump_table(fp, 0x57b1c, 0x5807e);
	fprintf(out, "\n");
	fprintf(out, "\n");
